...
import { buildSupportPrompt, RelevantKnowledge } from "@/lib/prompt";
      ...
      try {
        while (true) {
          ...
          for (const event of events) {
            ...
            try {
              const json = JSON.parse(lines) as { delta?: string; output_text?: string };
...
async function callGpt(prompt: string): Promise<ReadableStream<Uint8Array>> {
  ...
  if (!apiKey) throw new Error("未配置 OPENAI_API_KEY，请先配置环境变量");
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = buildSupportPrompt({
      ...
      relevantKnowledge: body.relevantKnowledge,
    });
